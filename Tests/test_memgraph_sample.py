...
def test_create_sample_memgraph_data():
    ...
    with driver.session() as session:
        ...
        """
        session.run(create_query)  # type: ignore

        # 5. Assert the graph was created successfully
        print("[INFO] Verifying created nodes...")
        result = session.run("MATCH (n:Concept) RETURN count(n) as count")  # type: ignore
        record = result.single()
        assert record is not None, "No record returned for concept count query."
        count = record["count"]
        ...
        nodes = session.run("MATCH (n:Concept) RETURN n.name as name, n.category as category LIMIT 3")  # type: ignore
