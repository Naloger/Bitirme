...
from services.Agentic.AgentMiddlewares.LoopGuardMiddleware.Functions._detect_loop import (
    _detect_loop,
)
from services.Agentic.AgentMiddlewares.LoopGuardMiddleware.Functions._extract_stream_parts import (
    _extract_stream_parts,
)
from services.Agentic.AgentMiddlewares.LoopGuardMiddleware.Functions._generate_feedback_prompt import (
    _generate_feedback_prompt,
)
from services.Agentic.AgentMiddlewares.LoopGuardMiddleware.Functions._messages_to_ollama_format import (
    _messages_to_ollama_format,
)
from services.Agentic.AgentMiddlewares.LoopGuardMiddleware.Functions._normalize_chunk import (
    _normalize_chunk,
)
from services.Agentic.AgentMiddlewares.LoopGuardMiddleware.Functions._salvage_answer_from_thinking import (
    _salvage_answer_from_thinking,
)
from services.Agentic.AgentMiddlewares.LoopGuardMiddleware.Functions._stream_chat_once import (
    _stream_chat_once,
)
from services.Agentic.AgentMiddlewares.LoopGuardMiddleware.state import StreamGuardState
...
class StreamGuardMiddleware(AgentMiddleware):
    ...
    def __init__(self, **kwargs):
        ...
        self.temperature = float(
            kwargs.get("temperature", getattr(default_config, "temperature", 0.7))
        )
        self.max_tokens = int(
            kwargs.get("max_tokens", getattr(default_config, "max_tokens", 8000))
        )
        self.timeout = float(
            kwargs.get("timeout", getattr(default_config, "timeout", 60.0))
        )
        self.max_loops = int(
            kwargs.get("max_loops", getattr(default_config, "max_loops", 3))
        )
